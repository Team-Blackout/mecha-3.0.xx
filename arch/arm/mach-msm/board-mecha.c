...
/*static int mecha_syn_ts_power(int on)
{
	...
	if (on) {
		id = PCOM_GPIO_CFG(MECHA_SYN_TP_INT_N, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_16MA);

		gpio_set_value(MECHA_GPIO_TP_EN, 1);
		gpio_set_value(MECHA__SYN_TP_RSTz, 0);
		msleep(10);
		gpio_set_value(MECHA__SYN_TP_RSTz, 1);
		msleep(100);
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);

	} else {
		id = PCOM_GPIO_CFG(MECHA_SYN_TP_INT_N, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_16MA);
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
		gpio_set_value(MECHA__SYN_TP_RSTz, 0);
		udelay(50);
	}
...
static struct i2c_board_info i2c_devices[] = {
	{
		...
		/*.irq = MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_TP_INT_N))*/
...
static int pm8058_gpios_init(void)
{
	...
	struct pm8xxx_gpio_init_info gpio01 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_TP_INT_N),
		{
...
/*	struct pm8xxx_gpio_init_info gpio02 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_TP_INT_N_CPU),
		{
...
*/
	...
	struct pm8xxx_gpio_init_info gpio18 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_AUD_EP_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
	...
	struct pm8xxx_gpio_init_info gpio19 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_AUD_SPK_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
...
/*	struct pm8xxx_gpio_init_info wimax_host_wakeup = {
		PM8058_GPIO_PM_TO_SYS(MECHA_WiMAX_HOST_WAKEUP),
		{
			.direction      = PM_GPIO_DIR_IN,
			.pull           = PM_GPIO_PULL_NO,
			.vin_sel        = PM8058_GPIO_VIN_S3,
...
*/
	struct pm8xxx_gpio_init_info kim_headset_gpios[] = {
		/*{
			PM8058_GPIO_PM_TO_SYS(MECHA_AUD_REMO_PRES),
			{
				.direction      = PM_GPIO_DIR_IN,
				.output_buffer  = 0,
				.output_value   = 0,
		...
		{
			PM8058_GPIO_PM_TO_SYS(MECHA_AUD_REMO_EN),
			{
				.direction      = PM_GPIO_DIR_OUT,
				.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
				.output_value   = 0,
...
/*        rc = pm8xxx_gpio_config(gpio02.gpio, &gpio02.config);
	...
	if (rc) {
		pr_err("%s MECHA_AUD_EP_EN config failed\n", __func__);
		return rc;
	}
	rc = pm8xxx_gpio_config(gpio19.gpio, &gpio19.config);
	if (rc) {
		pr_err("%s MECHA_AUD_SPK_EN config failed\n", __func__);
		return rc;
	}
...
//	.key_gpio		= PM8058_GPIO_PM_TO_SYS(MECHA_AUD_REMO_PRES),
//	.key_irq		= 0,
//	.key_enable_gpio	= PM8058_GPIO_PM_TO_SYS(MECHA_AUD_REMO_EN),
	.adc_mic		= 15672,
...
static struct kobj_attribute mecha_syn_virtual_keys_attr = {
	.attr = {
...
static void __init mecha_init(void)
{
	...
	if (skuid == 0x2a302) {
		for (rc = 0; rc < ARRAY_SIZE(i2c_devices); rc++) {
			if (!strcmp(i2c_devices[rc].type, SYNAPTICS_3K_INCELL_NAME)) {
				strcpy(i2c_devices[rc].type, ATMEL_QT602240_NAME);
				i2c_devices[rc].addr = 0x94;
				i2c_devices[rc].platform_data = &mecha_ts_atmel_data;
				i2c_devices[rc].irq = MSM_GPIO_TO_INT(MECHA_GPIO_TP_INT_N);
			}
		}
		mecha_properties_attrs[0] = 	&mecha_syn_virtual_keys_attr.attr;
	}
